...
export const handleFollowersList = async (): Promise<IResponse> => {
  const response = await Axios.get("/requests");
  return response.data;
};

export const handleAcceptFollow = async (id: number): Promise<IResponse> => {
  const response = await Axios.patch("/requests/accept/" + id);
  return response.data;
};

export const handleDeclineFollow = async (id: number): Promise<IResponse> => {
